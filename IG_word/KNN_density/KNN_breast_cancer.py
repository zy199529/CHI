...
def space_density_distance(trainingSet,instance1,instance2,length):
    ...
    for x in range(length):
        ...
        for z in range(len(trainingSet)):
            test_distance += math.pow((trainingSet[z][x]-instance1[x]), 2) / (trainingSet[z][x]+instance1[x])
        for y in range(len(trainingSet)):
            train_distance += math.pow((trainingSet[y][x]-instance2[x]), 2) / (trainingSet[y][x]+instance2[x])
        distance += (math.pow(instance1[x]-instance2[x], 2)/(instance1[x]+instance2[x]))*(u*math.sqrt(test_distance/(test_distance + train_distance)) + v*math.sqrt(test_distance/ train_distance))
